...
namespace epodreczniki
{
    ...
    public sealed partial class PrivacyPolicyPage : Page
    {
        ...
        public PrivacyPolicyPage()
        {
            ...
            this.policyWebView.Navigate(new Uri("ms-appx-web:///PrivacyPolicy.html"));            
        }
        ...
        {            
            ...
            if(accepted)
            { 
                this.acceptButton.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
                this.mainGrid.RowDefinitions[2].Height = new GridLength(0);
            }

            // jeśli jest to regulamin pokazany z widoku szczegółów podręcznika: pokaż przycisk "Back"
            if (e.NavigationParameter != null && e.NavigationParameter is bool && !((bool)e.NavigationParameter))
            {                
        ...
        {
            var messageDialog = new MessageDialog("Czy akceptujesz treść regulaminu?");

            messageDialog.Commands.Add(new UICommand("Tak", new UICommandInvokedHandler(this.CommandInvokedHandler), 1));
            messageDialog.Commands.Add(new UICommand("Nie", new UICommandInvokedHandler(this.CommandInvokedHandler), 0));
        ...
        private void policyWebView_ScriptNotify(object sender, NotifyEventArgs e)
        {
        ...
        {            
            if(command.Id is int && (int)command.Id == 1)
            {
                ...
                IEnumerable<CollectionDataItem> collections = null;
                try
                {
                    ...
                    var localSettings = ApplicationData.Current.LocalSettings;
                    if (localSettings != null)
                        localSettings.Values["PrivacyPolicyAccepted" + "_" + App.AppVersion()] = true;

                    // sprawdzenie czy jest coś do pokazania: pobranie listy podręczników
                    CollectionsDataSource.SetMaxWidth(Window.Current.Bounds);
                    collections = await CollectionsDataSource.GetCollectionsAsync();
                }
                catch
                {
                    collections = null;
                ...
                else
                {
                    ...
                    {
                        this.Frame.Navigate(typeof(OfflineInfoPage), null);
                    }
                }
        ...
        private async void policyWebView_DOMContentLoaded(WebView sender, WebViewDOMContentLoadedEventArgs args)
        {
            await policyWebView.InvokeScriptAsync("DocumentLoaded", null);
        }
